...
int main(int argc, char* argv[])
{
  ...
  for(int i = 0; i < (params.nx * params.ny); i++){
    if(obstacles[i]){
      obstacleIndices[obsSize] = i;
      obsSize++;
    }
    else{
      nonObstacleIndices[nobsSize] = i;
      nobsSize++;
    }
  }
  ...
  for (int jj = 0; jj < params.ny; jj++)
  {
    for (int ii = 0; ii < params.nx; ii++)
    {
      ...
      ** respecting periodic boundary conditions (wrap around) */
      ...
      propagateSwap(params, cells, tmp_cells, ii, jj, y_n, x_w, y_s, x_w);
    }
  }
...
float collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  ...
  ** are in the scratch-space grid */
  ...
  // for(oi i = 0; i < nobsSize; i++){
  //   int index = nonObstacleIndices[i];
